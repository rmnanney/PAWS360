...
import { Calendar, Clock, User, MessageSquare, CheckCircle, AlertCircle, BookOpen, GraduationCap, Mail, Phone } from "lucide-react";
...
const advisorDirectory = [
	{
		...
		specialties: ["General Education", "Transfer Students", "Academic Planning"],
				...
				<div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
					...
					<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
						<Card>
							...
							<CardContent>
								<div className="text-2xl font-bold">{degreeProgress.overallProgress}%</div>
								<p className="text-xs text-muted-foreground">
									{degreeProgress.completedCredits} of {degreeProgress.totalCredits} credits
								</p>
							</CardContent>
						</Card>
														...
														<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
															<div className="space-y-2">
																...
																<div className="flex items-center space-x-2">
																	...
																	<span className="text-sm">{advisor.availability}</span>
																</div>
															</div>
															<div>
									...
									<CardContent>
										<div className="space-y-4">
											<div className="flex items-center justify-between">
												...
												<span className="text-sm font-bold">{degreeProgress.overallProgress}%</span>
											</div>
											<div className="w-full bg-gray-200 rounded-full h-2">
												<div
													className={`h-2 rounded-full ${getProgressColor(degreeProgress.overallProgress)}`}
											...
											<div className="grid grid-cols-2 gap-4 text-sm">
												<div>
													...
													<p className="font-semibold">{degreeProgress.completedCredits}/{degreeProgress.totalCredits}</p>
												</div>
												<div>
													<p className="text-muted-foreground">Expected Graduation</p>
													<p className="font-semibold">{degreeProgress.expectedGraduation}</p>
												</div>
											</div>
										</div>
									</CardContent>
											...
											{degreeRequirements.map((req, index) => (
												<div key={index} className="space-y-2">
													<div className="flex items-center justify-between">
														...
														<Badge variant={req.status === 'Almost Complete' ? 'default' : 'secondary'}>
															{req.status}
														</Badge>
													</div>
