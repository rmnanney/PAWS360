...
require_cfg() {
    ...
    local value="$2"
    if [[ -z "${value}" ]]; then
        echo "ERROR: ${name} is required (set ${name} env variable)" >&2
        exit 1
    fi
}

require_cfg "PROMETHEUS_URL" "${PROMETHEUS_URL}"
require_cfg "GRAFANA_URL" "${GRAFANA_URL}"
require_cfg "LOKI_URL" "${LOKI_URL}"
